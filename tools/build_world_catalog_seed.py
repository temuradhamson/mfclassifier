...
WUXI_CHINA_2024_JSONL = ROOT / "data" / "wuxi-2024-lubricant-inspection.jsonl"
YANTAI_CHINA_2025_JSONL = ROOT / "data" / "yantai-2025-gasoline-detergent-inspection.jsonl"
...
def ecuador_inen_certified_record(row: dict) -> dict:
    """Convert one product-grade row from an official INEN announcement."""
    technical = row["technical"]
    ...
    generic = {
        ...
        "brand": row["brand"],
        "name": row["product_name"],
        "category": "Ecuador INEN certified lubricant announcement",
        "category_code": row["family_code"],
        "family": FAMILY_NAMES[row["family_code"]],
        "sae_class": technical["sae"][0],
        "api_class": "; ".join(performance),
    ...
    record["specifications"].update({
        ...
        "ecuador_inen_announcement_date": row["announcement_date"],
        "ecuador_inen_source_quality_flags": row["source_quality_flags"],
        "source_url": row["source_url"],
        "source_facts_sha256": row["source_facts_sha256"],
    })
    # These announcements do not expose a certificate number or expiry, so the
    # exact announcement row remains the evidence identity.
    record["canonical_key"] += f"|ecuador_inen_announcement:{normalize(row['source_record_id'])}"
    record["product_id"] = "WC-" + hashlib.sha256(record["canonical_key"].encode()).hexdigest()[:20]
    return record
...
def guatemala_siges_lubricant_record(row: dict) -> dict:
    """Convert one generic technical item from Guatemala's SIGES nomenclature."""
    technical = row["technical"]
    generic = {
        "id": row["source_record_id"],
        "source_number": row["source_record_id"],
        "brand": row["brand"],
        "name": row["product_name"],
        "category": "Guatemala SIGES government procurement item nomenclature",
        "category_code": row["family_code"],
        "family": FAMILY_NAMES[row["family_code"]],
        "sae_class": technical["sae"][0] if technical["sae"] else "",
        "api_class": "; ".join(f"API {value}" for value in technical["api_gl"]),
    ...
    record.update({
        ...
        "source_id": row["source_id"],
        "source_record_id": row["source_record_id"],
        "source_row": int(row["source_item_code"]),
        "evidence_status": row["evidence_status"],
        "lifecycle_status": row["lifecycle_status"],
        "snapshot_date": row["dataset_snapshot_date"],
    })
    record["specifications"].update({
        ...
        "dot_source_reported": technical["dot"],
        "nlgi_source_reported": technical["nlgi"],
        "guatemala_siges_budget_line": row["source_budget_line"],
        "guatemala_siges_item_name": row["source_item_name"],
        "guatemala_siges_characteristics": row["source_characteristics"],
        "guatemala_siges_presentations": row["source_presentations"],
        "guatemala_siges_catalog_updated_at": row["source_catalog_updated_at"],
        "guatemala_siges_source_quality_flags": row["source_quality_flags"],
        "source_url": row["source_url"],
        "source_facts_sha256": row["source_facts_sha256"],
    })
    record["codes"]["guatemala_siges_item_code"] = {
        "system": "GUATEMALA_SIGES_ITEM_CODE",
        "value": row["source_item_code"],
        "source_id": row["source_id"],
        "status": row["lifecycle_status"],
    }
    record["canonical_key"] += f"|guatemala_siges_item:{row['source_item_code']}"
    record["product_id"] = "WC-" + hashlib.sha256(record["canonical_key"].encode()).hexdigest()[:20]
    return record
...
def bolivia_ypfb_lubricant_record(row: dict) -> dict:
    """Convert one current official YPFB product/grade identity."""
    technical = row["technical"]
    performance = [
        *(f"API {value}" for value in technical["api"]),
        *(f"API {value}" for value in technical["api_gl"]),
    ...
    generic = {
        ...
        "brand": row["brand"],
        "name": row["product_name"],
        "category": "Current official YPFB Bolivia lubricant catalog",
        "category_code": row["family_code"],
        "family": FAMILY_NAMES[row["family_code"]],
        "sae_class": technical["sae_engine"] or technical["sae_gear"],
        "api_class": "; ".join(performance),
    ...
    record["specifications"].update({
        ...
        "application": row["application"],
        "thickener": row["thickener"],
        "source_url": row["source_url"],
        "source_page_modified": row["source_page_modified"],
        "source_page_facts_sha256": row["source_page_facts_sha256"],
        "technical_sheet_url": row["technical_sheet_url"],
        "technical_sheet_sha256": row["technical_sheet_sha256"],
        "safety_sheet_urls": row["safety_sheet_urls"],
        "safety_sheet_sha256": row["safety_sheet_sha256"],
        "catalog_url": row["catalog_url"],
        "catalog_sha256": row["catalog_sha256"],
        "source_quality_flags": row["source_quality_flags"],
    })
    record["canonical_key"] += f"|bolivia_ypfb_grade:{normalize(row['source_record_id'])}"
    record["product_id"] = "WC-" + hashlib.sha256(record["canonical_key"].encode()).hexdigest()[:20]
    return record
...
def rwanda_almc_current_record(row: dict) -> dict:
    """Convert one current ALMC Rwanda TDS product-grade identity."""
    specs = row["specifications"]
    performance = [
        *(f"API {value}" for value in specs.get("api", [])),
        *(f"API {value}" for value in specs.get("api_gl", [])),
    ...
    generic = {
        ...
        "brand": row["brand"],
        "name": row["product_name"],
        "category": "Current complete ALMC Rwanda TDS catalog",
        "category_code": row["family_code"],
        "family": FAMILY_NAMES[row["family_code"]],
        "sae_class": sae,
        "api_class": "; ".join(performance),
    ...
    record["specifications"].update({
        ...
        "no_package_price_stock_or_order_action_published": True,
    })
    record["canonical_key"] += (
        f"|rwanda_almc_current:{normalize(row['source_record_id'])}"
    )
    record["product_id"] = (
        "WC-" + hashlib.sha256(record["canonical_key"].encode()).hexdigest()[:20]
    )
...
def ethiopia_noc_caltex_record(row: dict) -> dict:
    """Convert one reviewed Caltex product/grade from NOC Ethiopia."""
    specs = row["specifications"]
    generic = {
        "id": row["source_record_id"],
        "source_number": row["source_record_id"],
        "brand": row["brand"],
        "name": row["product_name"],
        "category": "NOC Ethiopia recoverable Caltex lubricant archive",
        "category_code": row["family_code"],
        "family": FAMILY_NAMES[row["family_code"]],
        "sae_class": specs.get("sae_engine", specs.get("sae_gear", "")),
        "api_class": "/".join(specs.get("api", [])),
    ...
    record["canonical_key"] += (
        f"|ethiopia_noc_caltex:{normalize(row['source_record_id'])}"
    )
    record["product_id"] = (
        "WC-" + hashlib.sha256(record["canonical_key"].encode()).hexdigest()[:20]
    )
...
def merge_ethiopia_noc_caltex_evidence(
    target: dict, source_record: dict, raw: dict
) -> None:
    """Attach NOC Ethiopia archive evidence to a strict existing identity."""
    target_specs = target["specifications"]
    source_specs = source_record["specifications"]
    for key in (
...
def guyana_guyoil_lubricant_record(row: dict) -> dict:
    """Convert one explicitly identified GUYOIL-distributed lubricant."""
    technical = row["technical"]
    performance = [
        *(f"API {value}" for value in technical["api"]),
        *(f"API {value}" for value in technical["api_gl"]),
    ...
    generic = {
        ...
        "brand": row["brand"],
        "name": row["product_name"],
        "category": "Current official GUYOIL Guyana lubricant catalog",
        "category_code": row["family_code"],
        "family": FAMILY_NAMES[row["family_code"]],
        "sae_class": technical["sae_engine"] or technical["sae_gear"],
        "api_class": "; ".join(performance),
    ...
    record["specifications"].update({
        ...
        "packages_source_reported": technical["packages_source_reported"],
        "product_line": row["product_line"],
        "distributor_source_reported": row["distributor"],
        "source_name_spelling": row["source_name_spelling"],
        "source_url": row["source_url"],
        "source_page_modified": row["source_page_modified"],
        "source_page_api_sha256": row["source_page_api_sha256"],
        "source_page_facts_sha256": row["source_page_facts_sha256"],
        "source_quality_flags": row["source_quality_flags"],
    })
    record["canonical_key"] += f"|guyana_guyoil_product:{normalize(row['source_record_id'])}"
    record["product_id"] = "WC-" + hashlib.sha256(record["canonical_key"].encode()).hexdigest()[:20]
    return record
...
def trinidad_tobago_np_ultra_record(row: dict) -> dict:
    """Convert one current official NP ULTRA product-grade identity."""
    technical = row["technical"]
    performance = [
        *(f"API {value}" for value in technical["api"]),
        *(f"API {value}" for value in technical["api_gl"]),
    ...
    generic = {
        ...
        "brand": row["brand"],
        "name": row["product_name"],
        "category": "Current official NP ULTRA Trinidad and Tobago lubricant catalog",
        "category_code": row["family_code"],
        "family": FAMILY_NAMES[row["family_code"]],
        "sae_class": technical["sae_engine"] or technical["sae_gear"],
        "api_class": "; ".join(performance),
    ...
    record["specifications"].update({
        ...
        "source_page_text_sha256": row["source_page_text_sha256"],
        "source_document_urls": row["source_document_urls"],
        "source_document_sha256": row["source_document_sha256"],
        "source_facts_sha256": row["source_facts_sha256"],
        "source_quality_flags": row["source_quality_flags"],
    })
    record["canonical_key"] += f"|trinidad_tobago_np_ultra_product:{normalize(row['source_record_id'])}"
    record["product_id"] = "WC-" + hashlib.sha256(record["canonical_key"].encode()).hexdigest()[:20]
    return record
...
def nicaragua_lubrinsa_local_record(row: dict) -> dict:
    """Convert one local AUTO fluid from Lubrinsa's current Nicaragua catalog."""
    technical = row["technical"]
    generic = {
        "id": row["source_record_id"],
        "source_number": row["source_record_id"],
        "brand": row["brand"],
        "name": row["product_name"],
        "category": "Current Nicaragua local automotive fluid product card",
        "category_code": row["family_code"],
        "family": FAMILY_NAMES[row["family_code"]],
        "sae_class": "",
        "api_class": "",
    ...
    record["specifications"].update({
        "source_grade": technical["source_grade"],
        "source_product_name": row["source_product_name"],
        "package_source_reported": row["source_package"],
        "source_url": row["source_url"],
        "source_image_url": row["source_image_url"],
        "source_image_sha256": row["source_image_sha256"],
        "source_facts_sha256": row["source_facts_sha256"],
    ...
    record["canonical_key"] += f"|nicaragua_lubrinsa_local:{normalize(row['source_record_id'])}"
    record["product_id"] = "WC-" + hashlib.sha256(record["canonical_key"].encode()).hexdigest()[:20]
    return record
...
def fuchs_catalog_record(row: dict, source_id: str, market_name: str) -> dict:
    ...
    record["specifications"].update({
        ...
        "fuchs_content_identity_sha256": fuchs_content_identity_sha256(row),
        "fuchs_technical_identity_sha256": [fuchs_technical_identity_sha256(row)]
        if fuchs_technical_identity_sha256(row) else [],
    })
    source_key = {
...
def fuchs_content_identity_sha256(row: dict) -> str:
    """Hash exact current-catalog content while ignoring market publication dates."""
    empty_description_sha256 = hashlib.sha256(b"").hexdigest()
    description_hashes = sorted(
        value for value in row.get("source_description_sha256", [])
        if value and value != empty_description_sha256
...
def fuchs_technical_identity_sha256(row: dict) -> str:
    """Hash one strong technical revision without descriptions or page dates."""
    specifications = sorted(row.get("specifications", []))
    approvals = sorted(row.get("approvals", []))
    recommendations = sorted(row.get("fuchs_recommendations", []))
    technical = row.get("technical", {})
    technical_evidence = any(
        value not in (None, "", [], {})
        for field, value in technical.items()
        if field != "product_form"
    )
    if not specifications or not (approvals or recommendations) or not technical_evidence:
        return ""
    payload = {
...
def fuchs_identity_hashes_unique_to_name_family() -> dict[str, set[str]]:
    """Return reusable FUCHS hashes from globally conflict-free name/families."""
    empty_description_sha256 = hashlib.sha256(b"").hexdigest()
    description_identities: dict[str, set[tuple[str, str]]] = defaultdict(set)
    content_identities: dict[str, set[tuple[str, str]]] = defaultdict(set)
    technical_identities: dict[str, set[tuple[str, str]]] = defaultdict(set)
    records_by_identity: dict[tuple[str, str], list[dict]] = defaultdict(list)
    description_occurrences = Counter()
    content_occurrences = Counter()
    technical_occurrences = Counter()
    for path in sorted((ROOT / "data").glob("fuchs-*-products.jsonl")):
        for line in path.read_text(encoding="utf-8").splitlines():
            ...
            for value in row.get("source_description_sha256", []):
                if value and value != empty_description_sha256:
                    description_identities[value].add(identity)
                    description_occurrences[value] += 1
            content_identity = fuchs_content_identity_sha256(row)
            if content_identity:
                content_identities[content_identity].add(identity)
                content_occurrences[content_identity] += 1
            technical_identity = fuchs_technical_identity_sha256(row)
            if technical_identity:
                technical_identities[technical_identity].add(identity)
                technical_occurrences[technical_identity] += 1
    ...
    return {
        "description": {
            value for value, name_families in description_identities.items()
            if len(name_families) == 1 and description_occurrences[value] > 1
            and next(iter(name_families)) not in conflicting_identities
        },
        "content": {
            value for value, name_families in content_identities.items()
            if len(name_families) == 1 and content_occurrences[value] > 1
            and next(iter(name_families)) not in conflicting_identities
        },
        "technical": {
            value for value, name_families in technical_identities.items()
            if len(name_families) == 1 and technical_occurrences[value] > 1
            and next(iter(name_families)) not in conflicting_identities
        },
    }
...
def select_fuchs_identity_match(
    ...
    unique_description_hashes: set[str], unique_content_hashes: set[str],
    unique_technical_hashes: set[str],
    ...
    content_identity = fuchs_content_identity_sha256(raw)
    if content_identity in unique_content_hashes and not group_has_professional_conflict:
        content_matches = [
            row for row in matches
            if content_identity in as_value_list(row["specifications"].get("fuchs_content_identity_sha256", []))
            and not exclusive_duplicate_conflicts(row, source_record)
        ]
        if len(content_matches) == 1:
            return content_matches[0], "exact_content_identity"
    ...
    technical_identity = fuchs_technical_identity_sha256(raw)
    if technical_identity in unique_technical_hashes and not group_has_professional_conflict:
        technical_matches = [
            row for row in matches
            if set(as_value_list(row["specifications"].get("fuchs_technical_identity_sha256", [])))
            == {technical_identity}
            and not exclusive_duplicate_conflicts(row, source_record)
        ]
        if len(technical_matches) == 1:
            return technical_matches[0], "exact_technical_identity"
    return None, None
...
def merge_fuchs_catalog_evidence(target: dict, source_record: dict, raw: dict) -> None:
    ...
    for field, values in {
        ...
        "fuchs_content_identity_sha256": [fuchs_content_identity_sha256(raw)],
        "fuchs_technical_identity_sha256": [fuchs_technical_identity_sha256(raw)]
        if fuchs_technical_identity_sha256(raw) else [],
    }.items():
...
def integrate_fuchs_market(
    ...
    unique_description_hashes: set[str], unique_content_hashes: set[str],
    unique_technical_hashes: set[str],
    identity_match_counts: dict[str, Counter],
) -> dict:
    ...
    for raw, source_record in zip(source_rows, source_records):
        ...
        target, match_basis = select_fuchs_identity_match(
            matches, source_record, raw, unique_description_hashes, unique_content_hashes,
            unique_technical_hashes,
        )
        if target is not None:
            ...
            elif match_basis == "exact_content_identity":
                identity_match_counts["content"][source_id] += 1
            elif match_basis == "exact_technical_identity":
                identity_match_counts["technical"][source_id] += 1
...
def main() -> None:
    ...
    wuxi_china_source_rows = [
        json.loads(line)
        for line in WUXI_CHINA_2024_JSONL.read_text(encoding="utf-8").splitlines()
        if line
    ]
    wuxi_china_product_key = {}
    wuxi_china_products_matched_to_existing = 0
    wuxi_china_duplicate_occurrences_merged = 0
    wuxi_china_products_added = 0
    wuxi_created_keys = set()
    for raw in wuxi_china_source_rows:
        comparable_map = unique_inspection_target_map(china_inspection_comparable_identity)
        professional_map = unique_inspection_target_map(china_inspection_professional_identity)
        package_independent_map = unique_inspection_target_map(china_inspection_package_independent_identity)
        target = comparable_map.get(china_inspection_comparable_identity(raw))
        ...
        if target is not None:
            merge_china_inspection_evidence(target, raw)
            if target["canonical_key"] in wuxi_created_keys:
                wuxi_china_duplicate_occurrences_merged += 1
            else:
                wuxi_china_products_matched_to_existing += 1
        else:
            target = samr_china_inspection_record(raw)
            input_records.append(target)
            wuxi_created_keys.add(target["canonical_key"])
            wuxi_china_products_added += 1
        china_inspection_existing_entries.append((raw, target))
        wuxi_china_product_key[raw["source_record_id"]] = target["canonical_key"]
    yantai_china_source_rows = [
        json.loads(line)
        for line in YANTAI_CHINA_2025_JSONL.read_text(encoding="utf-8").splitlines()
        if line
    ]
    yantai_china_product_key = {}
    yantai_china_products_matched_to_existing = 0
    yantai_china_duplicate_occurrences_merged = 0
    yantai_china_products_added = 0
    yantai_created_keys = set()
    for raw in yantai_china_source_rows:
        comparable_map = unique_inspection_target_map(china_inspection_comparable_identity)
        professional_map = unique_inspection_target_map(china_inspection_professional_identity)
        package_independent_map = unique_inspection_target_map(china_inspection_package_independent_identity)
        target = comparable_map.get(china_inspection_comparable_identity(raw))
        ...
        if target is not None:
            merge_china_inspection_evidence(target, raw)
            if target["canonical_key"] in yantai_created_keys:
                yantai_china_duplicate_occurrences_merged += 1
            else:
                yantai_china_products_matched_to_existing += 1
        else:
            target = samr_china_inspection_record(raw)
            input_records.append(target)
            yantai_created_keys.add(target["canonical_key"])
            yantai_china_products_added += 1
        china_inspection_existing_entries.append((raw, target))
        yantai_china_product_key[raw["source_record_id"]] = target["canonical_key"]
    ...
    rwanda_almc_current_source_rows = [
        json.loads(line)
        for line in RWANDA_ALMC_CURRENT_JSONL.read_text(
            encoding="utf-8"
        ).splitlines()
        if line
    ]
    rwanda_almc_current_records = [
        rwanda_almc_current_record(row)
    ...
    barbados_sol_availability_rows = [
        json.loads(line)
        for line in BARBADOS_SOL_AVAILABILITY_JSONL.read_text(
            encoding="utf-8"
        ).splitlines()
        if line
    ]
    ...
    fuchs_unique_identity_hashes = fuchs_identity_hashes_unique_to_name_family()
    fuchs_unique_description_hashes = fuchs_unique_identity_hashes["description"]
    fuchs_unique_content_hashes = fuchs_unique_identity_hashes["content"]
    fuchs_unique_technical_hashes = fuchs_unique_identity_hashes["technical"]
    fuchs_identity_match_counts = {
        "payload": Counter(), "content": Counter(), "description": Counter(),
        "technical": Counter(),
    }
    ...
    for raw, source_record in zip(fuchs_india_source_rows, fuchs_india_records):
        ...
        target, match_basis = select_fuchs_identity_match(matches, source_record, raw, fuchs_unique_description_hashes, fuchs_unique_content_hashes, fuchs_unique_technical_hashes)
        if target is not None:
            if match_basis in {"exact_payload_identity", "exact_content_identity", "exact_description_identity", "exact_technical_identity"}:
    ...
    fuchs_mexico = integrate_fuchs_market(input_records, fuchs_mexico_source_rows, "FUCHS_MEXICO_PRODUCT_FINDER", "Mexico", prior_fuchs_rows, fuchs_unique_description_hashes, fuchs_unique_content_hashes, fuchs_unique_technical_hashes, fuchs_identity_match_counts)
    ...
    fuchs_south_africa = integrate_fuchs_market(input_records, fuchs_south_africa_source_rows, "FUCHS_SOUTH_AFRICA_PRODUCT_FINDER", "South Africa", prior_fuchs_rows + fuchs_mexico_source_rows, fuchs_unique_description_hashes, fuchs_unique_content_hashes, fuchs_unique_technical_hashes, fuchs_identity_match_counts)
    ...
    fuchs_brazil = integrate_fuchs_market(input_records, fuchs_brazil_source_rows, "FUCHS_BRAZIL_PRODUCT_FINDER", "Brazil", prior_fuchs_rows + fuchs_mexico_source_rows + fuchs_south_africa_source_rows, fuchs_unique_description_hashes, fuchs_unique_content_hashes, fuchs_unique_technical_hashes, fuchs_identity_match_counts)
    ...
    fuchs_hungary = integrate_fuchs_market(input_records, fuchs_hungary_source_rows, "FUCHS_HUNGARY_PRODUCT_FINDER", "Hungary", prior_fuchs_rows + fuchs_mexico_source_rows + fuchs_south_africa_source_rows + fuchs_brazil_source_rows + fuchs_norway_source_rows, fuchs_unique_description_hashes, fuchs_unique_content_hashes, fuchs_unique_technical_hashes, fuchs_identity_match_counts)
    ...
    for slug, source_path, source_id, market_name in FUCHS_ADDITIONAL_MARKETS:
        ...
        integration = integrate_fuchs_market(input_records, source_rows, source_id, market_name, additional_prior_rows, fuchs_unique_description_hashes, fuchs_unique_content_hashes, fuchs_unique_technical_hashes, fuchs_identity_match_counts)
    ...
    ethiopia_noc_caltex_source_rows = [
        json.loads(line)
        for line in ETHIOPIA_NOC_CALTEX_JSONL.read_text(
            encoding="utf-8"
        ).splitlines()
        if line
    ]
    ethiopia_noc_caltex_records = [
        ethiopia_noc_caltex_record(row)
        for row in ethiopia_noc_caltex_source_rows
    ]
    target_by_source_identity = {
        (row["source_id"], row["source_record_id"]): row
        for row in input_records
    }
    ethiopia_noc_caltex_product_key = {}
    ethiopia_noc_caltex_matched_to_existing = 0
    ethiopia_noc_caltex_products_added = 0
    for raw, source_record in zip(
        ethiopia_noc_caltex_source_rows, ethiopia_noc_caltex_records
    ):
        target_source_id = raw["existing_target_source_id"]
        target_source_record_id = raw[
            "existing_target_source_record_id"
        ]
        if target_source_id:
            ...
            if target is None:
                raise RuntimeError(
                    "NOC Ethiopia strict target disappeared: "
                    f"{target_source_id}/{target_source_record_id}"
                )
            if target["family_code"] != raw["family_code"]:
                raise RuntimeError(
                    "NOC Ethiopia strict target family drift: "
                    f"{target_source_id}/{target_source_record_id}"
                )
            merge_ethiopia_noc_caltex_evidence(
                target, source_record, raw
            )
            ethiopia_noc_caltex_matched_to_existing += 1
        else:
            target = source_record
            input_records.append(target)
            ethiopia_noc_caltex_products_added += 1
        ethiopia_noc_caltex_product_key[
            raw["source_record_id"]
        ] = target["canonical_key"]
    if (
        ethiopia_noc_caltex_matched_to_existing,
        ethiopia_noc_caltex_products_added,
    ) != (3, 32):
        raise RuntimeError(
            "NOC Ethiopia denominator changed: "
    ...
    for raw in sudan_tappco_source_rows:
        ...
        if link_key not in source_link_keys:
            source_links.append(link)
            source_link_keys.add(link_key)
    for raw in ethiopia_noc_caltex_source_rows:
        target = canonical_by_key[
            ethiopia_noc_caltex_product_key[raw["source_record_id"]]
        ]
        link = {
            "product_id": target["product_id"],
            "source_id": raw["source_id"],
    ...
    for raw in jilin_china_source_rows:
        ...
        if link_key not in source_link_keys:
            ...
            source_link_keys.add(link_key)
    for raw in wuxi_china_source_rows:
        target = canonical_by_key[wuxi_china_product_key[raw["source_record_id"]]]
        link = {
            "product_id": target["product_id"], "source_id": raw["source_id"],
            "source_record_id": raw["source_record_id"], "source_row": raw["source_row"],
            "relation": "official_government_product_inspection_observation",
        ...
        if link_key not in source_link_keys:
            ...
            source_link_keys.add(link_key)
    for raw in yantai_china_source_rows:
        target = canonical_by_key[yantai_china_product_key[raw["source_record_id"]]]
        link = {
            "product_id": target["product_id"], "source_id": raw["source_id"],
            "source_record_id": raw["source_record_id"], "source_row": raw["source_row"],
            "relation": "official_government_product_inspection_observation",
    ...
    fuchs_technical_matches_by_source = dict(sorted(fuchs_identity_match_counts["technical"].items()))
    report = {
        ...
        "wuxi_china_2024_input_sha256": hashlib.sha256(WUXI_CHINA_2024_JSONL.read_bytes()).hexdigest(),
        "yantai_china_2025_input_sha256": hashlib.sha256(YANTAI_CHINA_2025_JSONL.read_bytes()).hexdigest(),
        ...
        "wuxi_china_2024_source_rows": len(wuxi_china_source_rows),
        "wuxi_china_products_matched_to_existing": wuxi_china_products_matched_to_existing,
        "wuxi_china_duplicate_occurrences_merged": wuxi_china_duplicate_occurrences_merged,
        "wuxi_china_products_added": wuxi_china_products_added,
        "yantai_china_2025_source_rows": len(yantai_china_source_rows),
        "yantai_china_products_matched_to_existing": yantai_china_products_matched_to_existing,
        "yantai_china_duplicate_occurrences_merged": yantai_china_duplicate_occurrences_merged,
        "yantai_china_products_added": yantai_china_products_added,
        ...
        "ethiopia_noc_caltex_source_rows": len(
            ethiopia_noc_caltex_source_rows
        ),
        ...
        "fuchs_exact_content_identity_rows_matched": sum(fuchs_content_matches_by_source.values()),
        "fuchs_exact_content_identity_rows_matched_by_source": fuchs_content_matches_by_source,
        ...
        "fuchs_exact_technical_identity_rows_matched": sum(fuchs_technical_matches_by_source.values()),
        "fuchs_exact_technical_identity_rows_matched_by_source": fuchs_technical_matches_by_source,
        "fuchs_unique_description_hashes": len(fuchs_unique_description_hashes),
        "fuchs_unique_content_hashes": len(fuchs_unique_content_hashes),
        "fuchs_unique_technical_hashes": len(fuchs_unique_technical_hashes),
...
if __name__ == "__main__":
    if "--compress-only" in sys.argv:
        ...
        main()
        # Replace the process instead of merely returning from main().  CPython
        # may retain freed arenas after the large XLSX/catalog build; exec
        # guarantees that LZMA starts with a fresh heap and no transient OOM
        # overlap with the source matrices.
        os.execv(sys.executable, [sys.executable, __file__, "--compress-only"])
