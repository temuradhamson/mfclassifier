...
- Канонические карточки: **105 734**.
- Строки до каноникализации: 105 735.
- Активные предложения: 3 044.
- Дубли `canonical_key`: 0.
- Карточки с семейством, нормализованным названием, source-link и SHA-256 первичного источника: 105 734 / 105 734 / 105 734 / 105 734.
- С прямым URL первичного источника: 105 143; без производителя: 85.
- С явно помеченным fallback вместо самостоятельного бренда: 36 261.

Число 105 734 подтверждает размер текущего seed. Оно не доказывает, что все мировые продукты уже собраны.

## Покрытие профессиональными полями

| Код | Семейство | Карточки | Селектор/класс | Техническая база | Оба признака | Текущий gate |
|---|---|---:|---:|---:|---:|---|
| M | Моторные масла | 47 620 | 42 824 (89.93%) | 25 550 (53.65%) | 24 577 (51.61%) | 22 138 проблем |
...
- Quality issues: high — 311, medium — 68 509, low — 15.
- `keep_separate_specification_conflict`: 10 769 пар, 16 604 затронутых карточек.
- `review_fuchs_multi_registry_identity`: 2 892 пар, 483 затронутых карточек.
...
SHA-256 проверенной сжатой SQLite: `b91e0d4dc62aa6c3ce4cd63b7f8447397fb6cac0a83a633d4a56237d45dc0c1f`.
