...
Отдельный provenance-aware слой содержит 105 684 канонические продуктовые строки
...
Китайский inspection-контур дополнен полным муниципальным XLS Wuxi 2024:
все десять партий сохранены, восемь относятся к моторным и две — к
гидравлическим маслам. Семь моторных строк имеют подтверждённые API+SAE,
восьмая — только SAE; гидравлические строки нормализованы как `L-HM 46` и
`ISO VG 46`. Два наблюдения строго связаны с существующими карточками, восемь
создали новые, а результаты проверки партий не трактуются как текущие offers.
Полная HTML-таблица Yantai 2025 проверена по всем 350 строкам: 15 наблюдений
автомобильных бензиновых моющих присадок нормализованы в 10 product identities
по десяти изготовителям, пять повторных партий объединены с сохранением всех
source links; 335 нерелевантных промышленных товаров исключены.
Полный машиночитаемый архив мониторинга ANP 2017–2026 добавляет 11 026
наблюдений реальных рыночных образцов. Они сведены в 2 065 product-grade
identity по регистрации, названию, SAE и уровню характеристик: 1 043 identity
связаны с текущим реестром, а 1 022 исторических града добавлены отдельно
(799 с опубликованным номером регистрации и 223 без него). Такие строки имеют
исторический lifecycle и не увеличивают число подтверждённых активных
предложений. Для каждого наблюдения сохранены выпуск, файл, лист, строка и
SHA-256; 51 случай отсутствия пакета присадок и восемь пометок отсутствия
регистрации перенесены как факты источника, а не как вывод классификатора.
Отдельный координатный разбор трёх PDF-отчётов ANP 2024 добавляет ещё 1 425
исторических наблюдений: полные моторные таблицы на 782 и 462 строки и 181
опубликованную строку трансмиссионного отчёта. Последний заявляет 190 анализов,
но содержит только 181 товарную строку; расхождение −9 сохранено явно, без
синтетического достраивания. В PDF получилось 693 профессиональных identity:
416 уже встречались в XLSX, ещё 150 сопоставлены с текущим ANP, и только 127
добавлены как новые исторические product-grade карточки.
...
- `data/anp-brazil-monitoring-observations.jsonl` — 11 026 исторических
  наблюдений из всех 28 XLSX-приложений PML 2017–2026 с product-grade
  сопоставлением, lifecycle, quality flags и точной координатой источника;
- `data/anp-brazil-monitoring-pdf-observations.jsonl` — 1 425 строк полных
  товарных таблиц трёх PDF-отчётов ANP 2024; CNPJ, продавцы и места отбора не
  публикуются, а страница и хэш исходного PDF сохранены;
- `data/anp-brazil-monitoring-pdf-exceptions.jsonl` — 4 837 исторических
  наблюдений из 73 PDF-выпусков ANP 2007–2017 и 5 612 появлений проб в
  приложениях о несоответствии регистрации, маркировки или качества. Это явно
  помеченный слой исключений, а не полная совокупность отобранных проб;
...
- государственные проверки Китая 2019–2025 годов — 452 исторических события
  SAMR и надзора Шэньчжэня, сведённых к 432 карточкам: повторные проверки одного
  продукта связываются во времени; контрафактные/подозреваемые партии остаются
  только в аудите, а не превращаются в товарные предложения;
...
- `deliverables/Global_lubricants_catalog_registry.xlsx` — реестр 187 источников
  и статусы допуска;
...
python3 tools/ingest_anp_monitoring_history.py
python3 tools/ingest_anp_monitoring_pdf_history.py
python3 tools/ingest_anp_monitoring_pdf_exceptions_history.py
