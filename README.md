...
- `tools/ingest_eaeu_conformity_lubricants.py` — возобновляемый двухпроходный
  ...
  независимые поисковые срезы и REST-проекции загружаются с ограниченной
  ...
  диагностически обрезанные выгрузки остаются в `.cache` и не входят в текущие
  54 017 строк до завершения полного обхода и проверки качества;
- `deliverables/World_lubricants_catalog_seed.xlsx` — проверяемая выгрузка;
- `deliverables/Global_lubricants_catalog_registry.xlsx` — реестр 102 источников
  и статусы допуска;
